...
현재 정확 보고서에 결속된 실행 역할은 **0 / 23**이다. 같은 SHA-1의 개발 실행에서 8장 보관소 첫 인사 두 페이지는 읽혔지만 `보관/찾기` 전환에서 완료 대사 CHR 페이지의 상주권이 풀려 선택·취소 화면이 깨졌다. 따라서 최신 실행은 결함을 좁힌 진단 증거이며 `runtime-bound` 통과가 아니다. 사람 검토가 끝나 번역 바이트를 확정한 뒤 ROM을 다시 만들고, 그 exact artifact에 최종 실행 증거를 결속한다.
