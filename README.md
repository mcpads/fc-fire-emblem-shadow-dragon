...
`analyze-translation-coverage`는 현재 누적 ROM과 번역 입력을 함께 읽어 45개 화면을 일본어 대상 36개·원문 문자만 보존 5개·텍스트 없음 4개로 나누고, 일본어 대상 전부를 22개 번역 도메인에 연결한다. 22개 도메인은 모두 원천에 결속됐고 22개 전부 한국어 입력을 갖는다. 모든 소비자에 설치된 도메인은 8개다. 현재 수명을 계수한 5개 화면 중 장 종료 대사가 233/210칸으로 가장 크며 나머지 31개 화면은 계속 미계수 상태다. 이 보고서는 원문·번역문·글리프 문자를 싣지 않으며, 미계수 수명이 남아 있는 동안 전역 완료 판정에 쓰지 않는다.
