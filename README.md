...
전투는 원본 4 KiB 페이지와 현재 이름·병종·장비·지형·대사 레시피를 런타임에 합성하는 입력 일반형 로더까지 구현했다. 현재 정확한 ROM에서 사운드 테스트의 서로 다른 여섯 조합, 콜드 연속 일반 전투 두 건의 선택자 `0/62`, 엔딩 경로의 합성 비개입을 실행으로 확인했다. 모든 합성 반환점은 독립 Rust 계산과 실제 CHR-RAM 4,096바이트가 같았고 `$07DF`는 표시 수명 동안 유지된 뒤 각 반복·종료 경계에서 한 번만 정리됐다. 가족별 최대를 따로 더한 `134+39=173/210`은 임의 조합도 수용하는 보수적 안전 상한이다. 가능한 모델 튜플을 정확히 탐색한 실제 최대는 텍스트 131자이고, 보호 코드 39개를 전부 잡아도 `170/210`이다. 이 131자 증인을 합성·수명·이탈까지 기계적으로 통과했지만 화면의 한국어 판독성은 실패했다. 따라서 현재 핵심 관문은 공유 렌더러에서 번역용 코드와 보존할 영어·숫자 코드의 소유권을 분리하는 것이며, 게임 전반의 표시 완료나 배포 가능성을 주장하지 않는다.
