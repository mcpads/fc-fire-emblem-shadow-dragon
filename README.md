...
전투는 원본 4 KiB 페이지와 현재 이름·병종·장비·지형·대사 레시피를 런타임에 합성하는 입력 일반형 로더까지 구현했다. 콜드 경로의 이전 정확한 ROM에서는 연속 적 전투 두 건의 선택자 `0/62`, 독립 재구성 페이지와 실제 CHR-RAM 4,096바이트 일치, `$07DF`의 표시 수명 유지와 종료 상태 `33`의 단일 정리를 확인했다. 이후 일반 전투와 다른 상위 상태를 쓰는 사운드 테스트 공용 전투도 `$84=04`, `$05EE=0D`, `$7730=05`로 판정하고, 각 반복 전투의 원본 초기화 writer `07:$AC17`에서만 재합성을 다시 열도록 확장했다. 이 새 산출물은 사운드 테스트 반복 조합과 일반 전투 회귀를 아직 실행 재검증하지 않았다. 나머지 관측 조합, 최악 173칸 수명과 엔딩 비개입도 미검증이다. 현재 산출물은 게임 전반의 표시 완성도를 주장하지 않는다. 모든 도달 가능한 일본어 소비처, 최악의 동시 한글 작업 집합, 보호 영어와 주변 그래픽을 하나의 누적 ROM에서 닫기 전까지 배포 후보가 아니다.
