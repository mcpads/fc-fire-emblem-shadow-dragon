...
`build-kr-patch`는 지원 일본판 원본에서 mapper 165 설정·부대 목록 단계와 1·2장 도입부를 순서대로 다시 만들어 하나의 누적 개발 ROM을 낸다. 주 대사 번역 입력은 의미 있는 일본어 2,541줄이 모두 채워졌지만 사람 승인 완료는 0줄이며, 현재 설치 범위는 5레코드·29줄이다. 소스에 결속한 장 제목 25개도 모두 검토 대기 한글로 채웠고 그중 1·2장을 대사와 같은 수명 페이지에 설치했다. 최종 ROM의 1장 여섯 화면과 자연 지도 복귀, 2장 제목과 첫 대사에서 한글 제목·대사, 원본 영어 `MAP`·숫자·`※` 표식, 초상화·지도·창이 함께 정상 표시됐다. 시작 메뉴와 실제 유닛명은 아직 일본어이며 다음 누적 번역 대상이다.

설정 항목 `サウンド`, `アニメーション`, `ウエイトタイマー`를 각각 `사운드`, `애니메이션`, `대기시간`으로 바꾸는 페이지 전환 PoC와, 부대 목록 머리말 `なまえ`를 `이름`으로 바꾸는 혼합 문자 PoC를 제공한다. 두 화면은 각각 화면별 한글 페이지 A/B를 사용하고, 화면 내부의 `A → B → A` 선택과 이탈 뒤 원본 CHR 복구를 확인했다. 현재 부대 목록은 한글 `이름`과 원본 `LV`·`HP`·숫자를 보존하지만 실제 유닛명은 아직 일본어다. 공유 유닛명 표는 부대 목록·상태창·전투 소비자를 함께 묶어 번역한다. 이로써 동적 글꼴 공급 관문 G3를 통과했지만 아직 정식 패치나 배포 후보는 아니다.
