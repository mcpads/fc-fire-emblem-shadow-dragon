...
`plan-full-translation-installation`은 지원 일본판 원본과 누적 후보를 결속해 선언된 14개 번역 도메인, 504개 대사 레코드, 928개 페이지 작업집합을 하나의 통합 개발 ROM에 설치한다. 현재 정확 출력 SHA-1은 `AF942D786D82FF8A5D0BFFD6AA3900B83176F7A1`, 스키마 31 보고서 SHA-1은 `1521DF006753C9EDE3FB9019951C7B6B02062E2D`이다. 이 14개 도메인의 기술 설치는 통과했지만 23개 번역 도메인의 소비 경로 전수 조사가 끝났다는 뜻은 아니다. 소스에 결속한 알려진 경로는 **5 / 23**이고, 전수 조사까지 완료한 도메인은 **0 / 23**다. `terrain_names`도 확인한 `battle_animation` 경로만 결속했으며, 나머지 **18 / 23**은 알려진 경로도 미해결이다. 23개 모두 전수 조사를 계속하며 이 수치는 기술 설치·실행 검증·사람 검토를 대신하지 않는다. 원본 영어 `MAP`, `LV`, `HP`, `STR`, `SKI`, `WLV`, `AGI`, `DEF`, `MOV`, `H.P`, `EXP`, `NEXT STORY`와 숫자는 보존한다. `{EA}`가 화자명 앞에 합성하던 일본어 표식 `9E AB`는 두 빈 타일로 투영한다.
