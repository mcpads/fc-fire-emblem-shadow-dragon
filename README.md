...
`build-kr-patch`는 지원 일본판 원본에서 검증한 표면을 순서대로 다시 만드는 11단계 누적 개발 ROM을 낸다. 설정·부대 목록, 1·2장 도입부, 시작·기록 메뉴, 아군명, 자동 병종 소개, 무기점, 전투, 7장 `C0:18` 최대 대사에 이어 원본 에셋 기반 한글 제목 로고를 설치한다. 현재 출력 SHA-1은 `1ddc435b5e678c214c2273ce7a665ba4bf0ade13`이며 42대사 레코드·168번역 줄을 포함한다. 원본 영어 `MAP`, `LV`, `HP`, `STR`, `SKI`, `WLV`, `AGI`, `DEF`, `MOV`, `H.P`, `EXP`, `NEXT STORY`, 숫자와 `※`는 보존한다.

`analyze-translation-coverage`는 45개 화면을 일본어 대상 36개·원문 문자만 보존 5개·텍스트 없음 4개로 나누고, 일본어 대상 전부를 22개 번역 도메인에 연결한다. 22개 도메인은 모두 원천과 한국어 입력에 결속됐고 모든 소비자에 설치된 도메인은 9개다. 일본어 대상 수명 36개를 전부 계수했으며, 글꼴 화면의 전역 최악은 저장 질문·전원 종료 안내·중단 메시지의 `209/210`, 별도 그래픽 예산을 쓰는 제목은 `117/121`이다. 수용량 관문은 닫혔지만 남은 13개 도메인의 소비자 설치와 최종 ROM의 일본어 0건 검증은 아직 끝나지 않았다.
