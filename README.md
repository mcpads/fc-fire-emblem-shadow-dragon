...
`plan-full-translation-installation`은 지원 일본판 원본과 누적 후보를 결속해 선언된 14개 번역 도메인, 504개 대사 레코드, 928개 페이지 작업집합을 하나의 통합 개발 ROM에 설치한다. 현재 정확 출력 SHA-1은 `DA0E9188C95F95283714EE74D39CF22BF280EF48`, 스키마 34 보고서 SHA-1은 `02F6E982B07F7498F00F21641FA211ED593E2014`다. 기술 설치는 통과했지만 소비 경로 전수 조사, 최종 산출물 실행 결속과 사람 검토를 대신하지 않는다. 직접 합성 상태 `02..26`의 37개는 모두 명시적인 페이지 정책에 결속됐다. 상태 `11/17`은 원본 handler와 생산자 가족 전체가 글리프 없이 `ED/EF` 제어만 만드는 경우로 한정해 현재 페이지를 유지하고, 상태 `25`의 원본 유닛 선택 도움말 6줄은 unit-command 페이지로 번역·설치한다. 품목·유닛·적군·병종 이름은 화면별 우회가 아니라 원본 공용 appender 진입점에서 카탈로그 페이지를 게시한다. 원본 영어 `MAP`, `LV`, `HP`, `STR`, `SKI`, `WLV`, `AGI`, `DEF`, `MOV`, `H.P`, `EXP`, `NEXT STORY`와 숫자는 보존한다. `{EA}`가 화자명 앞에 합성하던 일본어 표식 `9E AB`는 두 빈 타일로 투영한다.
...
현재 정확 보고서에 결속된 실행 역할은 **0 / 23**이다. 다만 같은 exact ROM의 한 개발 실행에서 콜드 부팅·1장 도입·지도, 부대 목록, 아군·적군 상세, 소지품 목록·행동·취소, 빈 보관소·취소와 무기점 품목·취소를 묶어 회귀했고 공용 이름 브리지의 실제 적중을 확인했다. 12장에는 별도 SaveRAM 파일, 무기점에는 좌표 진단 개입을 사용했으므로 이 결과는 통합 결함을 찾는 개발 회귀이지 `runtime-bound` 통과가 아니다. 사람 검토가 끝나 번역 바이트를 확정한 뒤 ROM을 다시 만들고, 그 exact artifact에 최종 실행 증거를 결속한다.
