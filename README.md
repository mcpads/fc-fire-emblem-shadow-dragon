...
직전 산출물에서 선언된 실행 완료는 현재 산출물에 자동 승계하지 않는다. 현재 `150BA426…` 보고서는 최종 실행 manifest를 아직 받지 않았으므로 실행 완료를 승격하지 않는다. 별도 exact-ROM 실행 장부에서는 게임이 만든 8장 SaveRAM만 주입한 콜드 경로로 명단의 `이름`, 유닛명·`LV/HP`·숫자·보더, B 지도 복귀와 진행 메뉴 재개방을 확인했다. 직전 exact에서 확인한 보관소, 전투 보상, 패배·사망, 장 전환과 엔딩은 영향 분석 근거로 남지만 현재 ROM의 실행 성공으로 자동 계상하지 않는다.
