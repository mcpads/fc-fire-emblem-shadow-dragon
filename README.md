...
현재 매퍼 변환의 우선 후보는 MMC2식 CHR 래치와 MMC3식 PRG·SRAM을 함께 제공하는 mapper 165다. 무번역 프로브는 원본 CHR을 보존한 채 재배치하고, 관측한 FD/FE 쌍용 변형 페이지 1개를 자동 생성한다. 타이틀·자동 능력치·1장 인트로 대화 표본은 원본 화면과 정확히 같고, 전투 애니메이션의 직접 오른쪽 `02/06` 쌍도 트리거 보정 없이 호환됨을 확인했다. 남은 직접 writer와 저장·불러오기·전체 진행 검증이 필요하며 정식 패치나 배포 후보가 아니다.
