...
현재 정확 보고서에서 선언된 실행 역할을 모두 충족한 도메인은 **2 / 23**이다. 같은 exact ROM의 콜드·세이브스테이트 미사용 실행 2회에서 유닛 UI, 소지품 목록·행동·전달·버리기, 저장 후속 선택창의 8개 역할과 32개 이미지 표본을 결속했다. 저장 후속 화면에서 깨졌던 `화 / 걸고걸`은 이제 대사와 공용 선택 글리프가 함께 상주해 `예 / 아니오`로 표시된다. 이 실행은 해당 목적별 묶음을 닫지만 다른 저장 분기, 전투, 패배, 엔딩과 나머지 소비자를 대신하지 않는다. 번역이나 설치 바이트를 고치면 영향을 받은 실행 증거만 새 exact artifact에 다시 결속한다.
