...
`plan-full-translation-installation`은 지원 일본판 원본과 누적 후보를 결속해 선언된 14개 번역 도메인, 504개 대사 레코드, 928개 페이지 작업집합을 하나의 통합 개발 ROM에 설치한다. 현재 정확 출력 SHA-1은 `EA8094066938C6762C339DB148D91A38E0153DB0`, 스키마 33 보고서 SHA-1은 `71FCEE6F0C072403C1575B01AA2416D6D89418F8`이다. 이 14개 도메인의 기술 설치는 통과했지만 23개 번역 도메인의 소비 경로 전수 조사가 끝났다는 뜻은 아니다. 소스에 결속한 알려진 경로는 **5 / 23**이고, 전수 조사까지 완료한 도메인은 **0 / 23**다. `terrain_names`도 확인한 `battle_animation` 경로만 결속했으며, 나머지 **18 / 23**은 알려진 경로도 미해결이다. 23개 모두 전수 조사를 계속하며 이 수치는 기술 설치·실행 검증·사람 검토를 대신하지 않는다. 화면 상주 정책은 직접 합성 상태 37개를 `19 central / 10 delegated / 8 unresolved`로 나누며, 전투 후 결과 `08`과 `NEXT STORY` `10`은 이전 번역 route를 지우고 원본 page 0을 명시적으로 선택한다. 원본 영어 `MAP`, `LV`, `HP`, `STR`, `SKI`, `WLV`, `AGI`, `DEF`, `MOV`, `H.P`, `EXP`, `NEXT STORY`와 숫자는 보존한다. `{EA}`가 화자명 앞에 합성하던 일본어 표식 `9E AB`는 두 빈 타일로 투영한다.
...
현재 정확 보고서에 결속된 실행 역할은 **0 / 23**이다. 같은 SHA-1의 개발 실행에서 8장 보관소 첫 인사 두 페이지는 읽혔지만 `보관/찾기` 전환에서 완료 대사 CHR 페이지의 상주권이 풀려 선택·취소 화면이 깨졌다. 작업 트리는 완료 페이지 보류 상태까지 결속했지만, 합성 상태 게시 뒤의 재투영 위치가 아직 유효한 통합 산출물로 통과하지 않았다. 따라서 최신 실행은 결함을 좁힌 진단 증거이며 `runtime-bound` 통과가 아니다. 사람 검토가 끝나 번역 바이트를 확정한 뒤 ROM을 다시 만들고, 그 exact artifact에 최종 실행 증거를 결속한다.
