...
`build-kr-patch`는 지원 일본판 원본에서 검증한 표면을 순서대로 다시 만드는 11단계 누적 개발 ROM을 낸다. 설정·부대 목록, 1·2장 도입부, 시작·기록 메뉴, 아군명, 자동 병종 소개, 무기점, 전투, 7장 `C0:18` 최대 대사에 이어 원본 에셋 기반 한글 제목 로고를 설치한다. 현재 출력 SHA-1은 `c114fd7fab2a87784754643b230e3690ede85a40`이며 42대사 레코드·168번역 줄을 포함한다. 원본 영어 `MAP`, `LV`, `HP`, `STR`, `SKI`, `WLV`, `AGI`, `DEF`, `MOV`, `H.P`, `EXP`, `NEXT STORY`, 숫자와 `※`는 보존한다.
...
제목 그래픽은 원본 다섯 행의 최대 `27×5` 영역에서 마지막 행의 두 `TM` 셀을 제외한 134셀만 교체한다. 원본 일본어 로고 전용 코드는 121개이고, 원본 화면 기반 ImageGen 로고를 200픽셀 폭으로 결정적으로 줄인 자산은 고유 타일 117개를 쓴다. 완성 연출이 칼 교차부 11셀을 원작 타일로 다시 쓰는 별도 명령도 같은 한국어 로고 셀을 쓰도록 Expected Write로 설치한다. 누적 ROM은 원본 칼, 두 셀 `TM`, `©1990 Nintendo`와 점멸 팔레트를 보존하며 프레임 1206·1331·1458의 초기·완성·저작권 위상과 1912의 자동 병종 소개 전환을 정확한 출력 해시에 결속했다. 사람 시각 승인과 패배 뒤 제목 복귀는 아직 별도 관문이다.
