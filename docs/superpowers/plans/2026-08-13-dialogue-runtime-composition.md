...
지금은 사이클 모델이 «비용을 모르는 호출»을 거부한다. 다음 과제는 그 두 도우미의 최악 사이클을 에뮬레이터로 재고, 그 값을 넣은 뒤 CHR 뱅킹을 다시 붙이는 것이다. 그 전에는 첫 한글 페이지가 화면에 나오지 않는다.

## 자체 검토

**1. 스펙 대응.** 설계의 «합성 절차» 절에서 `cold`의 두 단계 중 **원본 글꼴 복원**이 이 계획에 없다. 첫 페이지를 보는 데는 필요하지 않지만(덮어쓰지 않은 타일이 무엇이든 그 페이지가 쓰지 않는 코드다), 두 번째 페이지부터는 직전 잔재가 남는다. 계획 C의 첫 과제로 넘긴다. 이 계획의 산출물이 «첫 페이지»인 이유가 이것이다.
