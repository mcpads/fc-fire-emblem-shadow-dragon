...
같은 원본과 입력은 같은 ROM과 보고서 바이트를 만들어야 한다. 구조만 옮긴 리팩터링은 의미 교정이 모두 반영된 바로 직전 기준선과 누적·통합 ROM의 byte-identical 비교를 통과해야 한다. 기존 결함을 고친 의미 변경은 구조 이동과 별도 커밋으로 만들고, 이전 결함의 재현·새 ROM 해시·행동 시험·실행 증거를 함께 기록한다. 보고서 스키마나 문구만 의도적으로 바꾸었다면 ROM 동일성과 보고서 변화 이유를 분리해 기록한다.
