...
//!
//! **아직 범위가 너무 넓다.** 실행해 보니 합성이 끝난 뒤 배경 패턴 테이블 전체가
//! CHR RAM으로 바뀌어 맵 타일 자리에까지 한글이 나온다. 한글 타일이 실제로 렌더링된
//! 것은 파이프라인이 끝까지 도는 증거지만, `ready`는 «타일이 올라갔다»는 뜻이지
//! «지금 대사 창을 그리는 중»이라는 뜻이 아니다.
//!
//! 매퍼 165는 FD/FE 래치로 4 KiB CHR를 두 벌 두고 타일 인출 중에 갈아 끼운다. 원본은
//! 그 래치로 대사 글꼴과 맵 타일을 함께 쓴다. 지금 selector가 레지스터 2와 4를 모두
//! CHR RAM으로 돌려서 두 벌이 다 바뀌었다. 다음 과제는 대사 창이 인출하는 쪽 한 벌만
//! 돌리는 것이고, 그러려면 원본이 두 벌을 어떻게 나눠 쓰는지부터 확인해야 한다.
//!
