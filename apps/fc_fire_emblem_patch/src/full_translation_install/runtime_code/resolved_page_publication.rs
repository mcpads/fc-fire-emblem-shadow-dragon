...
//! 뒤 해야 할 일은 같다. 완성 기반이 없으면 원본 4 KiB 복원과 현재 레시피 덮기를
//! 요청하고, 기반이 있으면 복원 없이 현재 레시피만 덮는다. 새 페이지가 실제로 쓰는
//! 코드를 전부 다시 쓰므로 과거 그룹 번호를 비교할 필요가 없다. 해석 실패는 생산자가
//! 미리 써 둔 `inactive`를 유지한다.
//!
//! 입력 계약은 둘이다. 캐리는 해석 성공 여부이고 A는 해석 전 상주권이다. 초기
//! 생산자가 준비 상태 밖에서 들어왔으면 A에 `NO_RESIDENT_PAGE_RECIPE`를 넣는다. 현재
//! 상주권 표식은 `0x00`, 없음은 `0xFF`라 서로 충돌하지 않는다.
...
/// 새 가시 페이지를 해석한 뒤 이전 상주 페이지의 유무에 따라 요청 종류를 고른다.
///
/// 입력 A는 해석 전 상주권이다. `FF`면 완성 기반이 없으므로 원본 4 KiB 복원부터
/// 시작하는 cold를 반환한다. 그 밖에는 resolver가 이미 세운 대상 레시피 항목 커서를
...
mod tests {
    ...
    fn publication() -> RuntimeRoutine {
        build_resolved_page_publication(PUBLICATION_ORIGIN, PAGE_RECIPE_REQUEST_INITIALIZER)
            .unwrap()
    }
