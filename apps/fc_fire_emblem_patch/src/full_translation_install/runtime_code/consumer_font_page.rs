...
//!
//! 이 모듈은 그 두 수명을 분리한다. `$E690`과 이름 appender는 `$07FD`에 현재 페이지를
//! 게시하면서 즉시 오른쪽 FD를 고른다. bank 0B의 유일한 복합 UI 열기 호출은 같은
//! 페이지를 다시 적용한 뒤 게시값을 소비하고, 유일한 닫기 호출은 게시값을 지운 뒤
//! 원본 그래픽 페이지 복원을 그대로 수행한다. 전역 selector 사슬은 이 상태를 읽지
//! 않으므로 다른 CHR 생산자가 UI 상태를 상속할 수 없다.
...
const COMPOSITE_PAGE_ROUTES: [(u8, FontPageRole); 7] = [
    ...
    (ENDING_RECORD_COMPOSITE_STATE, FontPageRole::EndingRecord),
];

impl FontPageRole {
    fn mapper_register(self, pages: ConsumerFontPageRegisters) -> u8 {
        match self {
            Self::UnitCommand => pages.unit_command,
            Self::MapMenu => pages.map_menu,
            Self::EndingRecord => pages.ending_record,
...
impl ConsumerFontPageRegisters {
    fn all(self) -> [u8; 6] {
        [
            self.unit_command,
            self.map_menu,
    ...
    fn validate(self) -> Result<()> {
        let pages = self.all();
            ...
            "consumer font page uses the empty sentinel as a mapper register"
        );
        ensure!(
            pages.into_iter().collect::<BTreeSet<_>>().len() == pages.len(),
            "consumer font page maps two roles to the same mapper register"
        );
        ensure!(
            pages.iter().all(|page| *page & 0x03 == 0),
            "consumer font page is not an encoded 4 KiB extension page"
        );
        Ok(())
    }
}
...
/// A의 계획된 mapper register를 현재 UI 페이지로 게시하고 즉시 오른쪽 FD에 적용한다.
...
pub(super) fn build_composite_font_page_publisher(
    ...
    for page in [
        ...
        instructions.extend([
            Instruction::LdaImmediate(page.mapper_register(pages)),
...
/// bank 0B의 복합 UI 열기에서만 현재 페이지를 다시 적용한다. 게시값은 적용 직후
/// 지우며, 유효한 페이지가 없으면 원본 `LDA #0; JSR $C9BE` 의미로 돌아간다.
...
/// 복합 UI 닫기는 임시 페이지를 먼저 지운 뒤 원본 A를 그대로 중앙 FD 기록기에
/// 넘긴다. 이 경계 뒤에는 `$07FD`를 읽는 전역 selector가 없으므로 지형·전투·제목이
/// 마지막 UI 페이지를 상속하지 않는다.
pub(super) fn build_consumer_font_page_close(origin: u16) -> Result<RuntimeRoutine> {
    let instructions = [
        Instruction::Pha,
        Instruction::LdaImmediate(0),
        Instruction::StaAbsolute(CONSUMER_FONT_PAGE),
...
mod tests {
    ...
    fn pages() -> ConsumerFontPageRegisters {
            ...
            for _ in 0..256 {
                ...
                match opcode {
                    ...
                    0x20 => {
                        ...
                        if target == 0xFA58 {
                            self.selected_register = Some(self.a);
                        } else {
                    ...
                    0x8D => {
                        ...
                        if address == BANK_VALUE_REGISTER {
                            self.mapper_value = Some(self.a);
                        }
                    }
                    0x85 => {
        ...
        fn finish(self) -> (Box<[u8; 0x10000]>, RunResult) {
            (
                ...
                RunResult {
                    ...
                    mapper_value: self.mapper_value,
    ...
    fn static_redraw_maps_immediately_and_screen_close_clears_the_page() {
        let pages = pages();
        let activation =
            build_consumer_font_page_activation(ORIGIN, BANK_VALUE_REGISTER, pages).unwrap();
        ...
        let close = build_consumer_font_page_close(close_origin).unwrap();
        ...
        assert_eq!(first_draw.mapper_value, Some(pages.catalog[0]));
        ...
        assert_eq!(redraw.mapper_value, Some(pages.catalog[0]));
        assert_eq!(memory[usize::from(CONSUMER_FONT_PAGE)], pages.catalog[0]);
        ...
        assert_eq!(closed.mapper_value, None);
    ...
    fn dynamic_name_page_can_change_during_one_open_screen() {
        ...
        assert_eq!(first_name.mapper_value, Some(pages.catalog[1]));
        ...
        assert_eq!(next_name.mapper_value, Some(pages.catalog[0]));
        assert_eq!(memory[usize::from(CONSUMER_FONT_PAGE)], pages.catalog[0]);
    }
    ...
    fn unsupported_composite_clears_the_previous_screen_page() {
        ...
        assert_eq!(memory[usize::from(CONSUMER_FONT_PAGE)], 0);
        assert_eq!(result.mapper_value, None);
    }

    #[test]
    fn screen_open_reapplies_the_page_and_consumes_only_its_publication() {
        let pages = pages();
        let activation =
            build_consumer_font_page_activation(ORIGIN, BANK_VALUE_REGISTER, pages).unwrap();
        ...
        assert_eq!(result.selected_register, Some(RIGHT_FD_CHR_REGISTER));
        assert_eq!(result.mapper_value, Some(pages.unit_command));
    ...
    fn page_roles_cannot_share_the_empty_sentinel_or_each_other() {
        ...
        assert!(
            invalid
                ...
                .unwrap_err()
                .to_string()
                .contains("encoded 4 KiB extension page")
        );
    }
}
