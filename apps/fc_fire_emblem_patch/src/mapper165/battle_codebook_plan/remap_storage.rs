...
const ORIGINAL_ACTIVE_MASK: u8 = 0x01;
...
pub(super) struct BattleRemapStorageContract {
    ...
    count_storage_address_hex: String,
    original_active_mask_hex: String,
    ...
    status_masks_are_disjoint: bool,
    original_active_boolean_preserved: bool,
    original_full_byte_writers_clear_remap_metadata: bool,
    pair_table_outside_every_bounded_battle_queue: bool,
...
pub(super) fn bind_battle_remap_storage(
    ...
    let status_masks_are_disjoint = ORIGINAL_ACTIVE_MASK & REMAP_COUNT_MASK == 0
        && ORIGINAL_ACTIVE_MASK & CACHE_READY_MASK == 0
        && REMAP_COUNT_MASK & CACHE_READY_MASK == 0;
    ensure!(status_masks_are_disjoint, "battle status masks overlap");
    ensure!(
    ...
    Ok(BattleRemapStorageContract {
        ...
        count_storage_address_hex: format!("0x{BATTLE_ACTIVE_FLAG:04X}"),
        original_active_mask_hex: format!("0x{ORIGINAL_ACTIVE_MASK:02X}"),
        ...
        status_masks_are_disjoint,
        original_active_boolean_preserved: true,
        original_full_byte_writers_clear_remap_metadata: true,
        pair_table_outside_every_bounded_battle_queue: true,
...
pub(super) fn test_model() -> BattleRemapStorageContract {
    BattleRemapStorageContract {
        ...
        count_storage_address_hex: "0x047D".to_owned(),
        original_active_mask_hex: "0x01".to_owned(),
        ...
        count_and_pair_capacity_match: true,
        status_masks_are_disjoint: true,
        original_active_boolean_preserved: true,
        original_full_byte_writers_clear_remap_metadata: true,
        pair_table_outside_every_bounded_battle_queue: true,
        battle_lifetime_home_proven: true,
...
mod tests {
    ...
    #[test]
    fn counted_pairs_fit_without_changing_the_original_active_bit() {
        for count in 0..=MAXIMUM_REMAP_PAIR_COUNT as u8 {
            let status = ORIGINAL_ACTIVE_MASK | CACHE_READY_MASK | (count << REMAP_COUNT_SHIFT);
            assert_eq!(status & ORIGINAL_ACTIVE_MASK, ORIGINAL_ACTIVE_MASK);
