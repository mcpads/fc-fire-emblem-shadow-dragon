...
pub(crate) fn build_battle_text_cache_base(
    ...
    let glyphs = fixed.unique_glyphs().union(&dialogue.unique_glyphs()).copied().collect::<BTreeSet<_>>();
    let glyph_atlas = rasterize_atlas(&glyphs)?;
    ensure!(glyph_atlas.len() <= 8 * 1024, "battle glyph atlas exceeds one MMC3 PRG page");
    ...
    ensure!(output_rom.mapper() == OUTPUT_MAPPER, "battle cache base mapper changed");
    ensure!(output_rom.prg().len() == EXPANDED_PRG_SIZE, "battle cache base PRG size changed");
    ensure!(output_rom.chr() == parity_rom.chr(), "battle cache base CHR changed");
    ensure!(&output_rom.prg()[..parity_rom.prg().len()] == parity_rom.prg(), "battle cache base changed the parity PRG prefix");
    ...
    ensure!(active_fixed == parity_fixed, "expanded active fixed bank is not the mapper parity fixed bank");
    ...
    let report = BattleTextCacheBaseReport {
        ...
        output_sha1: output_sha1.clone(), output_mapper: output_rom.mapper(),
        prg_size: output_rom.prg().len(), chr_size: output_rom.chr().len(),
        glyph_count: glyphs.len(), glyph_atlas_byte_count: glyph_atlas.len(),
        glyph_atlas_sha1: sha1_hex(&glyph_atlas), glyph_atlas_mmc3_page: GLYPH_ATLAS_MMC3_PAGE,
        original_prg_prefix_preserved: true, active_fixed_bank_duplicate_sha1: sha1_hex(active_fixed),
        chr_sha1: sha1_hex(output_rom.chr()), glyph_characters_emitted: false,
        translation_text_emitted: false, runtime_cache_installed: false, release_eligible: false,
    ...
    let mut report_bytes = serde_json::to_vec_pretty(&report).context("serialize battle cache base report")?;
    report_bytes.push(b'\n');
    write_file(output_path, &output)?;
    write_file(report_path, &report_bytes)?;
    Ok(BattleTextCacheBaseSummary { output_sha1, report_sha1: sha1_hex(&report_bytes),
        glyph_count: glyphs.len(), glyph_atlas_byte_count: glyph_atlas.len() })
}

fn rasterize_atlas(glyphs: &BTreeSet<char>) -> Result<Vec<u8>> {
    let font = load_dalmoori()?;
    glyphs.iter().try_fold(Vec::with_capacity(glyphs.len() * GLYPH_TILE_SIZE), |mut atlas, glyph| {
...
fn expand_prg_with_atlas(parity_rom: &Rom, atlas: &[u8]) -> Result<Vec<u8>> {
    ensure!(parity_rom.prg().len() == 256 * 1024, "mapper parity PRG size changed");
    ...
    let atlas_end = GLYPH_ATLAS_PRG_OFFSET.checked_add(atlas.len()).context("glyph atlas range overflow")?;
    ...
    output.extend_from_slice(&header); output.extend_from_slice(&expanded_prg); output.extend_from_slice(parity_rom.chr());
    Ok(output)
}

fn write_file(path: &Path, data: &[u8]) -> Result<()> {
    if let Some(parent) = path.parent() { fs::create_dir_all(parent).with_context(|| format!("create {}", parent.display()))?; }
...
mod tests {
    ...
    fn expanded_prg_places_the_atlas_before_a_duplicate_fixed_bank() {
        ...
        image[..4].copy_from_slice(b"NES\x1A"); image[4] = 0x10; image[6] = 0x50; image[7] = 0xA0;
        ...
        assert_eq!(&expanded.prg()[GLYPH_ATLAS_PRG_OFFSET..GLYPH_ATLAS_PRG_OFFSET + 3], &[1, 2, 3]);
        assert!(expanded.prg()[EXPANDED_PRG_SIZE - FIXED_BANK_SIZE..].iter().all(|byte| *byte == 0xA5));
    }
}
