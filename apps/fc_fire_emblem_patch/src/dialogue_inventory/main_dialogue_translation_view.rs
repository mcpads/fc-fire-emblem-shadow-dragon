...
fn bind_transition_only_entry(
    ...
    const LAST_TRANSITION_ONLY_ENTRY: usize = 0x41;
    const SOURCE_SNIPPETS: &[(u16, &[u8], &str)] = &[
        (
            0xA12C,
            &[0xA9, 0x35, 0x8D, 0x3B, 0x77],
            "initialize ending candidate cursor",
        ),
        (
            0xA165,
            &[
                0xCE, 0x3B, 0x77, 0x20, 0x66, 0xA3, 0xAD, 0x3B, 0x77, 0x30, 0x41,
            ],
            "decrement and bound ending candidate cursor",
        ),
        (
            0xA195,
            &[0xAE, 0x3B, 0x77, 0xE8, 0x8A, 0x8D, 0xF1, 0x77],
            "derive ending entry from candidate cursor",
        ),
    ];
    ...
    for (address, expected, role) in SOURCE_SNIPPETS {
        ...
        let actual = source
            ...
            .with_context(|| format!("{role} is outside the source"))?;
        ensure!(
            actual == *expected,
            "{role} changed; transition-only ending entry reachability is no longer proven"
        );
