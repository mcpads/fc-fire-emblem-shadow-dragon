...
mod tests {
    ...
    #[test]
    fn source_has_twenty_two_profiles_and_preserves_automatic_screen_punctuation() {
        ...
        let entries = extract_source_entries(&rom).unwrap();

        assert_eq!(entries.len(), PROFILE_COUNT);
        assert_eq!(entries[0].description_lines.len(), 4);
        assert!(entries[0].description_lines[1].ends_with('。'));
        assert!(entries[0].description_lines[2].contains('、'));
    }

    #[test]
    fn public_workspace_translates_every_profile_within_source_owned_rows() {
        let source = Path::new(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../roms/Fire Emblem - Ankoku Ryuu to Hikari no Tsurugi (Japan).nes"
        ));
        ...
        let rom = Rom::from_path(source).unwrap();
        let plan = plan_class_profiles(&rom, workspace).unwrap();

        assert_eq!(plan.entries.len(), PROFILE_COUNT);
        assert_eq!(
            plan.entries
                .iter()
                .map(|entry| entry.description_line_logical_bytes.len())
                .sum::<usize>(),
            75
        );
        assert!(!plan.review_complete);
        assert!(
            plan.unique_glyphs().len() > crate::font_slots::ACTIVE_HANGUL_SLOT_COUNT,
            "the complete automatic sequence should require a consumer-aware page split"
        );
    }
}
